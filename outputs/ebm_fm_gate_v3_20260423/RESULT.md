...
| **FM gate v3 (seed=1)** | **0.3124** | **ep29** | **0.7130** | **0.7934** | **ep04** | **0.2585** |
...
| FM gate v2 (seed=1) | +0.2383 | 0.0014 | 0.7096 | 0.7885 | PASS |
| **FM gate v3 (seed=1)** | **+0.2585** | **0.0005** | **0.6946** | **0.8118** | **PASS** |

- v3 test ρ=0.2585 > v2(0.2383) ✅ — quality filter 효과 있음
- v3 test ρ=0.2585 < v1(0.2885) ❌ — test 챔피언은 여전히 v1
- **val-test 격차 추이**: v1(−0.009, test>val), v2(+0.046), v3(+0.054) — 버전마다 격차 확대 중
- v3 val ρ=0.3124는 N=177 소규모 val셋 + seed=1 운 성분이 포함됐을 가능성. test에서 0.054 하락
- C baseline(0.239±0.010) 대비 v3 test ρ=0.2585 초과 ✅ — FM gate 시리즈 전체가 C를 test에서 앞섬
...
> **"FM quality filter가 ep14 붕괴를 막아 val에서 v1/v2를 크게 앞섰다. 단 test에서는 v2(0.2383)는 넘었지만 v1(0.2885)은 못 넘었다. val-test 격차가 커지는 구조적 문제가 있고, 3-seed 재현이 필수다."**
...
### (3) val ρ=0.3124가 높았지만 test에서 0.2585로 떨어졌다

val에서 supervised B(0.3028)를 넘은 건 주목할 만했다. 그러나 test 결과가 나왔고 v3 test ρ=0.2585다.

val-test 격차 0.054는 v2(0.046)보다도 크다. v1은 오히려 test가 val보다 높았던(−0.009) 것과 정반대다. 이 패턴이 의미하는 건:

- v3 val ρ=0.3124는 ep29의 운 성분이 꽤 포함됐을 가능성이 높다. N=177 val셋에서 Spearman ρ의 표준오차는 크다.
- quality filter 덕분에 학습이 안정됐지만, 동시에 FM 샘플이 val 분포에 맞게 과적합된 방향으로 수렴했을 수 있다.
- 버전이 올라갈수록 val-test 격차가 커지는 건 구조적 신호다. 모델이 점점 val에 맞게 최적화되고 있다는 뜻.

**결론**: val 기준 "IRL > supervised" 주장은 아직 시기상조다. test에서 v1(0.2885)을 넘기 위해서는 3-seed 재현이 필요하다. 현재 v3 seed=1 단일 run으로는 v1 우위를 논문에서 주장할 수 없다.
...
| test ρ | **0.2885** | 0.2384 | 0.2585 |
| val-test 격차 | −0.009 | +0.046 | +0.054 |
| 첫 PASS epoch | ep12 | ep12 | **ep04** |
| ablation 완성도 | — | — | 미완 (3-seed 필요) |

test 챔피언은 여전히 v1(0.2885). v3가 v2(0.2384)는 넘었지만 v1은 못 넘었다. val-test 격차 확대 추세가 구조적 문제인지 seed 운인지는 3-seed 재현에서 판별된다.

---

## 6. 다음 스텝

- [x] **test set 평가**: v3 test ρ=0.2585 (p=0.0005), AUROC(E)=0.6946, ECE=0.8118. v2(0.2383) 초과 ✅, v1(0.2885) 미초과 ❌
- [ ] **seed 2, 3 재현 (다음 우선순위)**: 3-seed 평균으로 v1 대비 우위 여부 확정. val-test 격차 확대가 구조적 문제인지 판별. 논문 C와 같은 조건으로 비교하려면 필수
