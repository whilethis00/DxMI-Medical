# ebm_fm_gate_v1 — Result

## 1. 실험 메타

| 항목 | 내용 |
|------|------|
| **날짜** | 2026-04-21 |
| **베이스** | ebm_weighted_cd (C, 3-seed avg: ρ=0.239±0.010, AUROC=0.688 on test) |
| **목적** | FM gate로 SGLD warm-up 후 FM hybrid negative 투입 — trivial negative 문제 해소 + ranking 성능 향상 |
| **설정** | epochs=30, l2_reg=0.1, reward_cd_weight=1.0, fm_gate_sep_std_threshold=40→버그수정→재실행, fm_gate_warmup_steps=570, sgld_permanent_ratio=0.2, fm_gate_probe_size=16, seed=1 |
| **현황** | 학습 완료 (ep30) / val 완료 / **test set 평가 완료** |

---

## 2. 무엇을 검증하나

**맥락**: 원래 C(MaxEnt IRL)는 FM 샘플을 negative로 쓰다가 EBM이 real vs noise를 trivially 이진 분리하는 collapse(sep/std ≈ 130)가 발생했음. ebm_weighted_cd(C r1~r3)는 이 문제를 우회하기 위해 **의도적으로 SGLD-only**로 돌려 "EBM + reward-weighted CD" 핵심 메커니즘이 작동하는지 먼저 검증한 것. 그 결과 A < B < C (test ρ=0.239) 성립을 확인.

**이번 실험의 질문**: SGLD-only로 검증이 끝났으니, 이제 FM을 다시 투입해도 collapse 없이 성능을 더 올릴 수 있는가?

**접근**:
1. Gated hybrid negative: EBM이 충분히 학습된 후(sep/std EMA < threshold 연속 3회)에야 FM negative 투입
2. SGLD 20%는 전 기간 유지해 hard negative baseline 보존
3. FM gate 버그 수정 (`fm_gate_warmup_steps=570` 추가 — 초기 무작위 EBM에서 gate가 오작동하는 문제)

**합격 기준**: val Spearman ρ > 0.20 (p < 0.05), AUROC(E) > 0.65 — C best(0.239)에 근접하거나 초과

---

## 3. 학습 손실 곡선

### 주요 epoch 손실 요약 (second run 기준, 본 실험)

| epoch | step | rw | cd | reg | e+(std) | e-(std) | ∇rw | fm |
|-------|------|----|----|-----|---------|---------|-----|----|
| 1 | s0010 | -0.46 | -0.55 | 0.09 | -0.84(±0.06) | -0.30(±0.00) | 16.4 | off |
| 1 | s0190 | -4.94 | -9.77 | 4.83 | -4.78(±0.74) | +4.96(±0.01) | 4.0 | off |
| 2 | s0220 | +2.83 | -1.17 | 4.00 | -3.80(±0.15) | -2.64(±4.26) | 65.3 | **ON** ← gate 개방 |
| 5 | — | — | — | — | — | — | — | ON |
| 10 | s1900 | ≈-5.0 | ≈-10.0 | ≈5.0 | ≈-5.0(±0.3) | ≈+5.0(±0.0) | 2~10 | ON |
| 28 | s5320 | -4.99 | -10.05 | 5.06 | -4.95(±0.35) | +5.09(±0.09) | 2.3 | ON |
| 30 | s5700 | -4.86 | -9.43 | 4.57 | -4.58(±0.58) | +4.93(±0.04) | 7.8 | ON |

**관찰**:
- ep1 초반: e+ ≈ e- ≈ -0.3 (EBM 미학습 상태) → s0190에서 e+≈-5, e-≈+5로 빠르게 분리
- FM gate: ep02/s0220 (reward step ≈1100 > warmup 570)에서 개방. sep_std_ema가 threshold 조건 통과
- gate 개방 직후: ∇rw spike (65~372) 다수 발생 — FM 샘플이 SGLD와 혼합되면서 EBM에 harder signal
- ep10 이후: 손실 안정화, ∇rw 2~15 수준으로 정착. 간헐적 spike(50~600) 산발적으로 지속
- **sep=32.7 고착**: gate 개방 후 sep_std_ema 업데이트 중단 (gate 체크 로직이 disabled되기 때문) — 로깅 공백

### 첫 번째 실행 조기 종료 (버그 확인)

| step | 현상 | 판단 |
|------|------|------|
| ep01/s0040 | FM gate 오작동 개방 (sep_std=9.3, warmup 없음) | EBM 미학습 상태에서 sep_std 우연히 threshold 통과 |
| ep01/s0160 | ∇rw=486.54 (gradient spike) | FM noise 샘플이 대량 투입되며 불안정 |
| ep01/s0180 | e+=-0.41(±5.25) | demo energy 분산 폭발, 학습 붕괴 징후 |
| ep01 val | ρ=+0.052 FAIL | 조기 종료 판단, warmup 추가 후 재실행 |

---

## 4. 검증 지표

### Epoch별 val 결과

| epoch | Spearman ρ | p-value | AUROC(E) | ECE | 판정 | 비고 |
|-------|-----------|---------|----------|-----|------|------|
| 1 | -0.1223 | 0.1050 | 0.5015 | 0.7707 | FAIL | |
| 2 | +0.0040 | 0.9580 | 0.4696 | 0.6983 | FAIL | |
| 3 | +0.0209 | 0.7823 | 0.5116 | 0.5563 | FAIL | |
| 4 | +0.0141 | 0.8518 | 0.4985 | 0.7505 | FAIL | |
| 5 | +0.1355 | 0.0722 | 0.6107 | 0.7946 | FAIL | |
| 6 | +0.0970 | 0.1991 | 0.5561 | 0.7867 | FAIL | |
| 7 | -0.0460 | 0.5431 | 0.4779 | 0.7029 | FAIL | |
| 8 | +0.0888 | 0.2397 | 0.5180 | 0.7859 | FAIL | |
| 9 | +0.0131 | 0.8622 | 0.6087 | 0.7841 | FAIL | |
| 10 | +0.1390 | 0.0650 | 0.6189 | 0.7948 | FAIL | |
| 11 | +0.1200 | 0.1117 | 0.5545 | 0.7938 | FAIL | |
| 12 | +0.1742 | 0.0204 | 0.6044 | 0.7945 | **PASS** | 첫 PASS |
| 13 | +0.0780 | 0.3021 | 0.5319 | 0.7821 | FAIL | |
| 14 | +0.1481 | 0.0491 | 0.5641 | 0.7860 | PASS | |
| 15 | +0.0788 | 0.2974 | 0.5423 | 0.7273 | FAIL | |
| 16 | +0.1680 | 0.0254 | 0.5887 | 0.7935 | PASS | |
| 17 | +0.1884 | 0.0120 | 0.6845 | 0.7929 | PASS | |
| 18 | +0.2246 | 0.0026 | 0.6525 | 0.7944 | PASS | |
| 19 | +0.2609 | 0.0005 | 0.6676 | 0.7954 | PASS | |
| 20 | +0.0059 | 0.9379 | 0.5188 | 0.7963 | FAIL | 이상 하락 |
| 21 | +0.2187 | 0.0035 | 0.6484 | 0.7964 | PASS | |
| 22 | +0.1975 | 0.0084 | 0.7111 | 0.7946 | PASS | |
| 23 | +0.1875 | 0.0125 | 0.5986 | 0.7936 | PASS | |
| 24 | +0.1995 | 0.0078 | 0.5967 | 0.7820 | PASS | |
| 25 | +0.2114 | 0.0047 | 0.6319 | 0.7888 | PASS | |
| 26 | +0.1500 | 0.0463 | 0.5370 | 0.7909 | PASS | |
| 27 | +0.1623 | 0.0309 | 0.5749 | 0.7872 | PASS | |
| **28** | **+0.2791** | **0.0002** | **0.6693** | 0.7945 | **PASS** | **BEST val** |
| 29 | +0.2604 | 0.0005 | 0.6710 | 0.7900 | PASS | |
| 30 | +0.2115 | 0.0047 | 0.6657 | 0.7936 | PASS | |

### Test set 최종 결과 (N=178, best val ckpt=ep28)

| 실험 | test ρ | p-value | AUROC(E) | ECE | 판정 |
|------|--------|---------|----------|-----|------|
| **FM gate v1** | **+0.2885** | **0.0001** | **0.7214** | 0.8127 | **PASS** |

### 이전 실험 대비 비교 (val 기준)

| 실험 | best val ρ | best epoch | AUROC(E) | ECE | 비고 |
|------|-----------|-----------|----------|-----|------|
| A (no IRL) | -0.0878 | 10 | 0.3884 | 0.7956 | FAIL |
| B (supervised) | +0.3028 | 80 | 0.6719 | 0.2095 | PASS |
| C r1 (SGLD-only) | +0.2364 | 14 | 0.6653 | 0.7933 | PASS |
| C r2 | +0.2902 | 17 | 0.6767 | 0.7936 | PASS |
| C r3 | +0.2946 | 20 | 0.7074 | 0.7934 | PASS |
| **FM gate v1** | **+0.2791** | **28** | **0.6693** | 0.7945 | PASS |

- FM gate v1 best val ρ(0.2791)은 C r1(0.2364)을 상회하고 C r2(0.2902)에 근접 ✅
- 단, C r3 best(0.2946)에는 미치지 못함. **seed 1 단일 run 결과**이므로 직접 비교는 유보

### 이전 실험 대비 비교 (test 기준)

| 실험 | test ρ | p-value | AUROC(E) | ECE | 비고 |
|------|--------|---------|----------|-----|------|
| C (SGLD-only, 3-seed avg) | 0.239 ± 0.010 | — | 0.688 | — | PASS |
| **FM gate v1 (seed=1)** | **+0.2885** | **0.0001** | **0.7214** | 0.8127 | **PASS** |
| FM gate v2 (seed=1) | +0.2384 | 0.0014 | 0.7096 | 0.7885 | PASS |

- **test ρ=0.2885** — C 3-seed avg(0.239) 대비 +0.050 상회. v2(0.2384)보다도 높음 ✅
- AUROC(E)=0.7214 — C avg(0.688) 대비 개선. 임상 판별력 향상 확인 ✅
- ECE=0.8127 — C 및 v2 대비 소폭 높음. calibration은 여전히 미해결

---

## 5. 결과 해석 및 인사이트

### 정직한 한 줄 요약

> **"FM을 넣어도 죽지 않는다"를 보여줬다. "FM이 도움된다"는 아직 증명 전이다.**

이 둘을 혼동하면 안 된다. 아래에서 하나씩 풀어 설명한다.

---

### (1) 실제로 확인된 것

**FM gate 메커니즘 자체는 작동했다.**

원래 문제는 이랬다. FM 샘플을 negative로 쓰면 EBM이 "real nodule vs. Gaussian noise"를 너무 쉽게 구분해버린다(sep/std ≈ 130). 그러면 EBM이 ranking을 배우는 게 아니라 binary classification으로 collapse된다. 이걸 막으려고 "EBM이 충분히 학습된 다음에만 FM을 투입하자"는 gate를 설계했고, 실제로 ep02/s0220에서 정상적으로 열리고 이후 30 epoch 동안 collapse 없이 완주했다.

이것만으로도 이전 C(MaxEnt IRL, epoch 180, FAIL)와 비교하면 의미 있는 진전이다. 설계한 대로 작동했다는 건 사실이다.

**val ρ가 0.2791까지 올라왔다.**

C r1(0.2364)보다 높고, C r2/r3(0.290/0.295)에 근접한다. val 기준이긴 하지만 이 범위에 들어온 것 자체는 유의미하다.

---

### (2) 착각하기 쉬운 것 — 아직 증명되지 않은 것

**"FM이 도움됐다"는 결론을 낼 수 없다.**

지금 실험 구조를 보면, FM이 투입된 이후에도 SGLD 20%는 계속 돌아간다. 즉 학습에 기여하는 요소가 두 개(SGLD + FM)가 섞여 있고, 어느 쪽이 성능을 올렸는지 분리할 방법이 없다. C r1~r3가 SGLD-only baseline 역할을 하긴 하지만, seed가 다르고 완전히 동일한 조건이 아니다. FM-only 실험도 없다.

결론적으로, 지금은 "FM을 넣은 조합이 C r1보다 val에서 높게 나왔다"는 관찰만 있다. 이 숫자가 FM 덕분인지, SGLD 비율 조합이 우연히 잘 맞은 건지, seed 1이 유독 좋은 건지 알 수 없다.

논문에서 "FM gate가 성능을 높였다"는 문장을 쓰려면 통제된 ablation이 반드시 있어야 한다.

---

### (3) 진짜 위험 신호 — ep19→ep20 collapse

ep19에서 ρ=0.261로 올라갔다가 ep20에서 ρ=0.006으로 한 epoch 만에 붕괴했다가 ep21에서 다시 0.219로 회복됐다. 이걸 "단발성 노이즈"로 보고 넘기면 안 된다.

왜냐하면 이 패턴이 의미하는 건 이렇다: **시스템 어딘가에 아직 불안정 요소가 살아있다.** 이번엔 한 epoch 만에 회복됐지만, 다른 조건(다른 seed, 더 긴 학습)에서 같은 collapse가 오고 회복이 안 될 수도 있다. 실제로 ∇rw가 ep 전반에 걸쳐 간헐적으로 수백 단위로 튀는 현상(ep1~5: 최대 624, ep10 이후도 40~100)이 이 불안정의 증거다. "안정화됐다"는 말은 절대 쓰면 안 된다.

---

### (4) 구조적 문제 — FM을 blind 상태로 돌렸다

FM gate가 열린 ep02/s0220부터 ep30/s5700까지 로그에서 `sep=32.7`이 단 한 번도 바뀌지 않는다. 이게 왜 문제냐면, gate가 닫혀 있을 때만 sep_std_ema를 업데이트하도록 코드가 짜여 있어서, gate가 열린 순간부터 FM 샘플의 품질을 전혀 추적하지 못했기 때문이다.

쉽게 말하면 이렇다. FM이 투입된 이후 28 epoch 동안:
- FM 샘플이 점점 더 real nodule처럼 좋아지고 있는지
- 아니면 다시 trivial collapse 방향으로 가고 있는지

아무것도 모른 채로 학습을 돌린 것이다. 이건 로깅 문제가 아니라 구조 문제다. 다음 실험에서 반드시 gate 이후에도 sep_std_ema가 계속 업데이트되도록 바꿔야 한다.

---

### (5) calibration 실패 — 논문에서 공격받는다

ECE ≈ 0.79는 SGLD-only C와 똑같다. FM을 넣어도 calibration은 전혀 개선되지 않았다. B(supervised)의 ECE=0.232와 비교하면 격차가 크다.

ECE가 높다는 건 모델이 "어떤 샘플이 더 uncertain한지 순서는 맞추지만, 그 확률값 자체를 믿을 수 없다"는 뜻이다. 임상적으로 보면 "이 결절은 60% 확률로 불확실하다"는 수치를 신뢰할 수 없게 된다. 리뷰어가 이걸 보면 "EBM이 ranking만 되고 probability estimation은 실패했다"고 지적한다. 방어 논리를 미리 준비해야 한다.

다만 이건 FM gate 문제가 아니라 EBM 구조 자체의 특성일 가능성이 높다. CD loss는 에너지의 순서는 보장하지만 절대적인 scale을 보장하지 않는다. B가 ECE가 낮은 건 supervised training이라서다. Post-hoc temperature scaling을 먼저 시도해볼 수 있다. 재훈련 없이 지금 당장 할 수 있고, 이걸로 ECE가 잡히면 구조 문제가 아니라는 걸 확인할 수 있다.

---

### (6) 지금 실험의 등급

| 항목 | 평가 |
|------|------|
| 아이디어 (FM gate 설계) | A — 문제를 정확히 파악하고 합리적인 해법 |
| 구현 | B+ — 작동하지만 sep 모니터링 blind 구간 존재 |
| 검증 | C — ablation 없음, seed 1만, test set 평가 없음 |
| 해석 | B- — 이번 revision 전까지 "FM이 좋다"로 과장했음 |

논문 기준으로는 아직 약하다. 핵심은 ablation이다.

---

## 6. 다음 스텝

- [x] **test set 평가**: `eval_test.py --config configs/ebm_fm_gate_v1.yaml` — ckpt_epoch0028.pt 기준 → test ρ=0.2885, AUROC(E)=0.7214, ECE=0.8127
- [ ] **seed 2, 3 재현**: FM gate v1 구조적 버그(sep blind, ∇rw spike) 존재 → v3 확정 후 v3으로 3-seed 재현 예정. v1 단독 재현은 보류
- [x] **fm_gate_v2 설계**: 완료 — sep monitoring fix + energy_clamp=20.0. 그러나 v2에서 ep14 FM sample quality 붕괴 확인(fm_e=-6.2). energy_clamp는 이 문제 차단 불가 → v3 필요
- [x] **∇rw spike 원인 분석**: v2 ep14 로그 추적 완료. FM 샘플이 demo 에너지 영역에 침범(fm_e < 0)할 때 CD loss 역전 → ∇rw 폭발. 해법: FM sample quality filter (fm_e < 0이면 step drop)
- [x] **ECE 개선**: temperature scaling 시도 — T=403(degenerate, T→∞). val/test mal_var>0 비율 80%로 모두 0.5 예측이 ECE 최소화. CD loss 구조 한계. 논문 방어: "ranking(Spearman ρ) primary, calibration은 CD loss의 known limitation"

---

## 7. 저장 파일 목록

```
outputs/ebm_fm_gate_v1_20260421/
├── train.log           # 전체 학습 로그 (2-run: 버그 발견 run + 본 run)
├── RESULT.md           # 이 파일
├── ckpt_epoch0028.pt   # best val checkpoint (ρ=0.2791) — git 제외
└── ckpt_epoch0030.pt   # last checkpoint — git 제외
```
