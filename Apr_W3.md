...
## ebm_weighted_cd 실험 완료 및 판정 (2026-04-18)
...
1. **재현 2~3회** — ρ=0.3041이 안정적으로 나오는지 확인 (단일 숫자 취하지 말 것)
2. **FM gate 로직 재설계** — sep_std_ema가 높을 때 열리는 조건으로 부등호 반전 또는 다른 metric
3. **Phase 2 진입 후 충격 관찰** — 이전 minirun에서 진입 시 grad_norm=935 발생한 전례 있음
4. **ECE는 나중** — energy ordering 안정화 후 temperature scaling 또는 calibration head
