...
def _parse_val_logs(log_file: Path) -> list[dict]:
    vals = []
...
def _parse_test_log(test_file: Path) -> dict | None:
...
def _write_result_md(cfg: dict, config_path: str, exp_dir: Path):
    ...
    test_file = exp_dir / "test.log"

    vals = _parse_val_logs(log_file)
    test = _parse_test_log(test_file)
